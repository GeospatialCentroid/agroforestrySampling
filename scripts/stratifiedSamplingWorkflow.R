...
write_csv(result1, "data/derived/samplingXTesting/aggregated/mlra80_iterations.csv")
...
for(i in c("10","16","20")){
  export <- paste0("data/derived/samplingXTesting/aggregated/mlra80_",i,".csv")
...
create_lm_fit <- function(data, response_col) {
  ...
  data %>%
    # We rename the column provided in response_col to a temporary,
    # consistent name 'response'. This makes the lm() formula simple.
