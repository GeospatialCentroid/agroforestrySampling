...
process_segmentations <- function(r, seed_list, output_dir, file_id, compactness = 0.2) {
  ...
  for (i in seq_along(seed_list)) {
    ...
    # Run SNIC
    segmentation <- snic::snic(r_clean, seeds = current_seed, compactness = compactness)
...
inspect_seed_density <- function(r, seed_list, seed_name, pt_col = "red", pt_cex = 0.6) {
  ...
  seed_vect <- tryCatch({
    ...
    if (terra::crs(v) == "") { terra::crs(v) <- terra::crs(r) }
    v
  }, error = function(e) {
    # Fallback: assume it is a matrix/df of coordinates (x, y)
    # create vector points and assign the raster's CRS
    terra::vect(as.matrix(current_seeds), type = "points", crs = terra::crs(r))
  })
  
  # 3. Plot Base Raster
  if (terra::nlyr(r) >= 3) {
    terra::plotRGB(r, r = 1, g = 2, b = 3, stretch = "lin", 
